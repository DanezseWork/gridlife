...
import type { PlannedFrequency } from "@/lib/api";
...
interface ScheduleFieldsProps {
  timing: "scheduled" | "recurring";
  scheduledDate: string;
  onScheduledDateChange: (value: string) => void;
  minScheduledDate: string;
  frequency: PlannedFrequency;
  onFrequencyChange: (value: PlannedFrequency) => void;
  weeklyDays: number[];
            ...
            {yearlyDays.map((entry, index) => (
              <div key={`${entry.month}-${entry.day}-${index}`} className="flex gap-2">
                ...
                <button
                  type="button"
                  onClick={() =>
                    onYearlyDaysChange(yearlyDays.filter((_, itemIndex) => itemIndex !== index))
                  }
